...
async def error_window(clb: CallbackQuery, selected_date: str) -> None:
...
def check_in_date_validator(manager: DialogManager, selected_date: str) -> bool:
    check_out_date: str | None = manager.dialog_data.get("check_out_date")
    today: str = str(date.today())
    if check_out_date:
        if selected_date < today or selected_date > check_out_date:
            return False
